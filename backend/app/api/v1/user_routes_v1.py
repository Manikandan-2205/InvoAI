...
from app.core.result import Result   # ✅ Import FIRST
...
@router.get("/get-all-user-details", summary="Get all users")
...
async def update_password(user_id: int, payload: PasswordUpdate, service: UserService = Depends(get_user_service)):
    try:
        ...
        result = await service.update_password(user_id, payload)
        if not result.success:
            return ApiResponse.error(result.message, result.code)
        return ApiResponse.success(message=result.message, code=result.code, data=result.data)
    except Exception as e:
