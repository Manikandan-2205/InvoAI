...
async def vendor_list(self) -> Result:
    try:
        ...
        vendor_list = [GlobalVendorList.from_orm(v).dict() for v in vendors.data]

        if vendor_list is None:
            Result.Fail(message="No Data Found!.",code=400)
        return Result.Ok(data=vendor_list, message="Active vendors fetched successfully", code=200)
